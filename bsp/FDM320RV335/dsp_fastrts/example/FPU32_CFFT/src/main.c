...
//
//#############################################################################
// $Copyright: Copyright (C) 2018 FDMTEK -
//              ALL RIGHTS RESERVED $
//#############################################################################
...
//! against MATLAB code under 2837x_CFFT/matlab/CFFTforrv335New.
//!
...
void cfft_f32_phasec(CFFT_F32_STRUCT_Handle hnd)
 {
     // Locals
    uint16_t i,j,k;
    uint16_t N    = hnd->FFTSize ;//FFT_getSize(hnd);
    fsize_t *phase = hnd->OutPtr;//FFT_getPhaseBufferPtr(hnd);
    fsize_t *io    = hnd->InPtr ;      //FFT_getIOBuffer2Ptr(hnd);
    {
...
int16_t main(void)
{
...
#else
    CFFT_f32_brev(hnd_cfft);            // asm function
    CFFT_f32(hnd_cfft);                    // Calculate FFT
#endif //USE_TABLES

    // Check the output
    for(i = 0; i < 2*CFFT_SIZE; i++){
        if(fabs(CFFTgoldenOut[i] - CFFToutBuff[i]) <= EPSILON){
            pass++;
        }else{
            fail++;
        }
    ...
    hnd_cfft->CurrentOutPtr = CFFTin1Buff; // Mag function output pointer
#ifdef __FDM320RV_TMU__ //defined when --tmu_support=tmu0 in the project
                           // properties
    ...
    // the phase()
    hnd_cfft->OutPtr=CFFTin1Buff;
    hnd_cfft->InPtr   = CFFToutBuff;
...
#else
    // Calculate phase, result stored in CurrentOutPtr
    volatile float dd = atan2(hnd_cfft->InPtr[59], hnd_cfft->InPtr[58]);

    CFFT_f32_phase(hnd_cfft);
//    cfft_f32_phasec(hnd_cfft);
#endif
    // Check the output
    for(i = 0; i < CFFT_SIZE; i++){
        if(fabs(CFFTgoldenPhase[i] - hnd_cfft->OutPtr[i]) <= EPSILON){
            pass++;
        }else{
            fail++;
        }
