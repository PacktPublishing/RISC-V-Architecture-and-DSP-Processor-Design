...
#pragma DATA_SECTION("firfilt");
#endif
int32_t dbuffer[FIR_ORDER+SIGNAL_LENGTH];
...
uint16_t FIXPT_DSP_runTest()
{
    ...
    fir.order       = FIR_ORDER+1;
    fir.dbuffer_ptr = &dbuffer[FIR_ORDER+SIGNAL_LENGTH+1];
...
#elif ((RUN_TEST == TEST_3) || (RUN_TEST == TEST_4) || (RUN_TEST == TEST_7))

    // zero out sigOut
    memset(sigOut, 0, sizeof(sigOut));
    // Initialize FIR32 object
    fir.order       = FIR_ORDER;
    fir.dbuffer_ptr = &dbuffer[FIR_ORDER+SIGNAL_LENGTH];
    fir.coeff_ptr   =(int32_t *)coeff;
    fir.init(&fir);
