...
uint16_t FIXPT_DSP_runTest()
{
    ...
    FFT32_brev2(ipcbsrc, ipcb, FFT_SIZE/2);         // bit-reversing
    ...
    // Bit reverse order the input
    FFT32_brev2(ipcbsrc, ipcb, FFT_SIZE/2);
    ...
    //Bit reverse the windowed input to get it back to regular order
//    FFT32_brev(ipcb, ipcb, FFT_SIZE/2);
//    FFT32_brev(&ipcb[1], &ipcb[1], FFT_SIZE/2);
    ...
    // Bit reverse order the input
    FFT32_brev(ipcbsrc, ipcb, FFT_SIZE/2);
//    FFT32_brev(&ipcbsrc[1], &ipcb[1], FFT_SIZE/2);
    // Apply the window to bit-reversed input
    cfft.win(handleCFFT32);
//    FFT32_winc();
    //Bit reverse the windowed input to get it back to regular order
...
uint16_t FIXPT_DSP_genReport()
{
    ...
    goldPass = 2*FFT_SIZE;
    for(i = 0, j = 0; i < FFT_SIZE; i = i + 2, j++ ){
        err[i]   = labs(win[j] - ipcbsrc[i]);
        err[i+1] = labs(win[j] - ipcbsrc[i+1]);
    ...
    for(i = 0, j = FFT_SIZE/2; i < FFT_SIZE; i = i + 2, j++){
        err[i+FFT_SIZE]   = labs(win[FFT_SIZE - j - 1] - ipcbsrc[i + FFT_SIZE]);
        err[i+FFT_SIZE+1] = labs(win[FFT_SIZE - j - 1] - ipcbsrc[i + FFT_SIZE + 1]);
