/*******************************************************************************
 ! \file source/vector/sub_SP_CVxCV.s
 !
 ...
 ! \date   06/08/2022
  ...
  HISTORY:
    06/08/2022 - original (XingYuanyuan)
...
#		flw/fsw -> flw.pm/fsw.pm
		...
		.attribute		5, "rv32i2p0_m2p0_f2p0_c2p0_xsc"	# arch 的编号是5 ,rv32IMFC  
															# 2P0代表其基本ISA的默认版本  
		.file			"vector_subCVfromCV.c"				# emit filename FILE LOCAL symbol table  
		.globl			sub_SP_CVxCV						# declare global symbol , 声明全局符号  
		.p2align		1									# align to power of 2 ,后续代码按 power(2) 字节对齐  
		.type			sub_SP_CVxCV,@function				# 将类型设置为函数或对象
...
# main loop , calculate		y_re[i] = w_re[i] - x_re[i]
#							y_im[i] = w_im[i] - x_im[i]
# .LBB1_2:
		addi			a3, a3, -2
		...
		flw.pm			ft0, a1,4
		flw.pm			ft1, a2,4
		fsub.s			ft2, ft0, ft1						# ft0 = ft0 - ft1 , y_re[i] = w_re[i] - x_re[i]

		flw.pm			ft3, a1,4	
		flw.pm			ft5, a2,4	
		fsub.s			ft4, ft3, ft5						# ft0 = ft0 - ft1 , y_im[i] = w_im[i] - x_im[i]
...
# end of function sub_SP_CVxCV()
# =============================================================================

.Lfunc_end1:
		.size			sub_SP_CVxCV, .Lfunc_end1-sub_SP_CVxCV
