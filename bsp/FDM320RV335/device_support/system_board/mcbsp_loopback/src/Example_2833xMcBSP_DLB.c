...
#define McbspbSelect *((Uint16 *)0x40c0)

// Prototype statements for functions found within this file.
void InitXintf(void);
void mcbspa_xmit(int a, int b);
void mcbspb_xmit(int a, int b);
void error(void);
void McbspbEnable(void);
void McbspbDisable(void);
...
// For this example, enable the GPIO for McBSP-A
   InitMcbspaGpio();
// For this example, enable the GPIO for McBSP-B
   InitMcbspbGpio();
...
// Step 5. Initialize all the Zone if the Mcbspb is used:
   InitXintf();
   McbspbEnable();		  //Open Mcbspb Channal
...
// Step 6. User specific code, enable interrupts:
      if(WORD_SIZE == 8)             // Run a loopback test in 8-bit mode
      {
          ...
          while(1)
          {
              mcbspa_xmit(sdata1,sdata2);
			  mcbspb_xmit(sdata1,sdata2);
              sdata1++;
              ...
              rdata1 = McbspbRegs.DRR1.all;                 // read DRR1
              if(rdata1 != rdata1_point) error();
