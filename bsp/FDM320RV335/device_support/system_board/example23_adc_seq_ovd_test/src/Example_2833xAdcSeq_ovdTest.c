//###########################################################################
//
// 文件名:   Example_2833xAdcSeq_ovdTest.c
//
// 标题:  ADC级联过采样模式测试
//
// ASSUMPTIONS:
...
//    在级联过采样模式下使用序列发生器1，
//    采样速率为1/(3*40ns) =8.3MHz
//
//    运行一会程序然后观察内容的变化.
//
//       观察变量:
//          SampleTable - 加载的转换值.
//          GPIO60      - 在每次ADC序列发生器标志时翻转
//
...
// Start SEQ1
   ...
   {  // Take ADC data and log them in SampleTable array
     ...
     for (i=0; i<(BUF_SIZE/16); i++)
     {
       ...
       GpioDataRegs.GPBSET.bit.GPIO60 = 1;  // Set GPIO60 for monitoring  -optional
       ...
       while (AdcRegs.ADCST.bit.INT_SEQ1== 0){}
 	   GpioDataRegs.GPBCLEAR.bit.GPIO60 = 1;  // Clear GPIO60 for monitoring  -optional
       AdcRegs.ADCST.bit.INT_SEQ1_CLR = 1;
